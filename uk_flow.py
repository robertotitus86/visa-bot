from uk_preguntas import PREGUNTAS_UK, GRUPOS_UK
...
class UKSession:
    ...
    def get_pregunta(self):
        if self.pregunta_idx < len(PREGUNTAS_UK):
            return PREGUNTAS_UK[self.pregunta_idx]
        return None
    ...
    def siguiente_mensaje(self, respuesta_anterior=None):
        if self.fase == "inicio":
            self.fase = "cuantos"
            return (
                "Perfecto, voy a recopilar los datos para la *Visa del Reino Unido* "
                "(Standard Visitor). Lo hacemos paso a paso. 😊\n\n"
    ...
    def generar_reporte(self):
        ...
        lineas = [
            f"🇬🇧 *VISA REINO UNIDO — DATOS COMPLETOS*",
            f"📅 {ahora}",
        ...
        lineas.append(f"\n{'═' * 35}")
        lineas.append(f"✅ Datos visa UK completados.")
        return "\n".join(lineas)
...
def esta_en_sesion_uk(phone):
    s = sesiones_uk.get(phone)
    return s is not None and s.fase != "completo"


def iniciar_sesion_uk(phone):
    session = UKSession(phone)
    sesiones_uk[phone] = session
    return session.siguiente_mensaje()


def procesar_mensaje_uk(phone, mensaje):
    session = sesiones_uk.get(phone)
    if session is None:
        session = UKSession(phone)
        sesiones_uk[phone] = session
